...
beforeAll(async () => {
	...
	const mockDocument = {
		...
		body: { 
			appendChild: () => {},
			get innerText() { return "Integration log message content"; }
		},
		querySelectorAll: () => [],
		querySelector: () => null,
	...
	client = new ApertureClient({
		...
		onApprovalRequest: () => Promise.resolve({ 
			approved: true, 
...
test("routes tool calls from MCP to client and returns results", async () => {
	...
	const logs = response.result as Array<{ level: string; message: string; timestamp: number }>;
	expect(Array.isArray(logs)).toBe(true);
	
	const matchingLog = logs.find(log => log.message.includes("Integrative Test Log Payload"));
